---
layout: default
title: Home
---

## **Welcome to MiddleRaster's Math Musings!**

I love math, but I don't have all that much to say about it.

Here's my first post, on how we know that:
[
\\[
\int \frac{1}{x} \, dx = \ln|x| + C
\\]
](TheIntegralOfOneOverX.html) (click on the formula).

And here's an easy-to-remember proof of [Fermat's Little Theorem](FermatLittleTheorem.html).

A little bit of [retirement math](/mathmusings/RetirementMath.html) and a bit on [when it makes sense to open a Roth IRA](/mathmusings/ToRothOrNot.html)

Back to [MiddleRaster](https://middleraster.github.io)'s home page.
